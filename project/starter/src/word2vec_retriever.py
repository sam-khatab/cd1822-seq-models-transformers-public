...
class Word2VecRetriever:
    ...
    def build_index(self, corpus_texts: List[str]) -> None:
        ...
        processed_docs = [self._preprocess_text(doc) for doc in corpus_texts]
        
        # YOUR CODE HERE: Train Word2Vec model
        # Create Word2Vec instance with self parameters (vector_size, window, etc.)
        self.model = Word2Vec(sentences=processed_docs, vector_size=self.vector_size, window=self.window, min_count=self.min_count, sg=self.sg, epochs=self.epochs)
        ...
        self.corpus_vectors = [self._text_to_vector(doc) for doc in corpus_texts]
        
        # YOUR CODE HERE: Convert list to numpy array and store corpus_texts
        self.corpus_vectors = np.array(self.corpus_vectors)
    ...
    def retrieve(self, queries: List[str], k: int = 10) -> Dict[int, List[int]]:
        ...
        for q_idx, query in enumerate(queries):
            ...
            if query_vector is not None:
                ...
                similarities = cosine_similarity(query_vector.reshape(1, -1), self.corpus_vectors).flatten()
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"❌ Error: {e}")
>>>>>>> 878daa0bd33a67492a923c957c6720dc33045b64
